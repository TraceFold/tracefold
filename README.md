...
| Machine-checked | **117** | theorems in Lean, 12 of them counterexamples, out of 118 line-initial declarations in the Lean sources; the remaining one is an `axiom`, the statement assumed rather than proved, and it is named in the report. No `sorry`, the keyword standing in for a proof nobody wrote, so there are none. Proof rather than bounded model checking: nothing here is true only up to a scope · 18 August 2026 |
