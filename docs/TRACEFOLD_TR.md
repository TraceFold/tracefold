...
| Availability | The repository `github.com/TraceFold/tracefold` is public. As of 2026-08-26, a fresh anonymous clone built with zero errors and passed its full public test floor — 1,986 tests across 375 suites, 0 failures — at commit `cd001fa`. The TypeScript SDK is published on npm as `@mahirhir/tracefold` (0.1.0). *(§D.4: the floor has grown since this dated snapshot — `README.md`'s own ratcheted count as of 2026-08-30 is 2,664 probes / 470 suites + SDK 36, 0 fail, 7 skipped, and a same-day, 2026-08-31, unratcheted recount reads 2,721 / 476. This row is a dated snapshot, not a live status line, the same discipline §5.2 uses for the pinned M7 floor table.)* |
...
### D.3 — 2026-08-29 erratum (audit `req/912`; recorded here, **not applied to the body**)

This batch differs from D.1 and D.2 above: those record edits already made in place, and this one does not. The sentences named below still read exactly as they did before this entry was added. Each row states what the body currently says, what a same-day recount measured, and the code anchor the recount used. Anchors are symbol names, not line numbers.

1. **Job name (§7.4)**: the sentence "`lean-current` and `difftest-nightly` are implemented CI jobs" named a job id that did not exist — `.github/workflows/ci.yml` had a job doing that work under the id `lean-build` (steps "lake build (AC-061)", "sorry grep (AC-061)", "T1-T5 ... presence (AC-061)" — the content was already AC-061-complete; only the id was off). **Fixed at the implementation, not the paper**: the job was renamed `lean-build` → `lean-current` on 2026-08-29 in `.github/workflows/ci.yml` (step content unchanged; `.github/workflows/release.yml` is untouched — out of this entry's authorized write scope, and its own `lean-build` mention is a comment only, not a job-id reference), so the §7.4 sentence is now literally true. Closes `req/912` C38.
2. **CLI verb count (§2.6, the `gx-cli` table row)**: the row lists 13 verbs (`submit, plan, verify, commit, undo, cancel, escalation, receipt, replay, log, key, policy, serve`). Recounted 2026-08-29 against `enum Command` in `crates/gx-cli/src/main.rs`: **23 variants**. The 10 not in the row: `wrap, attach, demo, demo-notes-server, limits, confine, verdict-checkpoint, checkpoint, repair, draft`. Table not edited.
3. **CI tense** (frontmatter *Formal-methods status*; §4.2's Lean/differential-test row and CI-scope row; §5.5's table and its "can turn a pull request red automatically" sentence; §7.4's "on every push" and "the gates exist and run"; §7.5's CI-coverage sentence): all describe the differential test and the 16/17-crate gate in the present tense, as running now. As of 2026-08-29, every scheduled run against this repository since the last green run (`31898083900`, 2026-08-15T17:21:42Z) has been rejected by the scheduler — 2–5 seconds, no log produced, which is a refusal to run rather than a run that passed or failed. The public repository (`github.com/TraceFold/tracefold`) also carries no `.github/workflows/` directory, so a reader of the published artifact cannot see the workflow these sentences describe. Source: `req/908` (run-object annotations; the billing-block finding) and `req/912` §3-1 C07/C09 (independent confirmation of both facts via `gh api`). **Not applied to the body**: resuming CI is a billing action gated on the Owner, and the sentences above are left as written pending that.
4. **Payload table (§3.1, "The payload, in full")**: the table lists 11 rows (`receipt_kind, transformation, verdict, canonical_cid, inclusion_proof, precondition_fingerprint, postcondition_fingerprint, inverse_delta, enforced, key_id, issued_at`), of which `issued_at` is already noted (row's own text, and §4.2) as living outside `ReceiptPayload`. Recounted 2026-08-29 against the `pub` fields of `ReceiptPayload` in `crates/gx-witness/src/receipt.rs`: **17 named fields**. The 7 not in the table, with their declared types: `confinement: Option<ConfinementContext>`, `catalogue_hash: Option<String>`, `read_set: Option<ReadSet>`, `reversibility: Option<Reversibility>`, `determinism_boundary: DeterminismBoundary`, `fingerprint_scope: String`, `fail_posture_engaged: bool`. This entry records only that these fields exist and their declared types — it does not state what any of them mean on the wire. `reversibility` in particular sits inside an open wording question (`req/912`'s B2/F1) that belongs to the Owner to settle, not to this entry. Table not edited.
5. **Endpoint count (§2.4, "Thirteen HTTP endpoints") — resolved same day.** Recounted 2026-08-29 directly against `crates/gx-api/src/lib.rs`: 23 `.route(` call sites, 22 distinct path strings, 25 distinct (method, path) pairs (`/candidates` carries both GET and POST). The figure 15 reported by the same-day audit (`req/912` C19) was itself an undercount: a single-line regex missed the seven `.route(` calls whose path string sits on the following line (`/candidates/{id}/escalation`, `/transformations/{id}/undo`, `/transformations/{id}/replay`, `/verdict-checkpoints`, `/verdict-checkpoints/{window_end}`, `/attach-sources`, `/attach-sources/{id}/observations`) — the counting instrument, not the router, was wrong. So the body's "thirteen" understates by nine distinct paths, and §2.4 uses the count as an argument ("thirteen opportunities for a second clock"), which is why this row exists. Body left unedited; the durable fix is deriving the count from the router rather than writing it by hand (`req/912` A-912-2).

### D.4 — 2026-08-31 stale audit (P-band sweep; recorded per the same discipline as D.1–D.3)

This batch is mixed, like D.1/D.2 for the two items applied to the body and like D.3 for the two logged only. Ground truth for each row: the Zenodo API record for the DOI below, `README.md`'s own ratchet line, and a direct recount against the named source file — not a digest of any of them.

1. **Formal publication (end-of-draft line) — applied to the body.** "Formal publication ... is gated on the author" was true through 2026-08-29 and stopped being true on 2026-08-30: a DOI now exists, `10.5281/zenodo.22168558`, confirmed 2026-08-31 by an anonymous read of the Zenodo record (`https://zenodo.org/api/records/22168558`, publication date 2026-08-30). Edited in place because this is a report catching up to an already-public fact, not a publication decision made by this entry. The [PB]→[PV] citation-promotion leg of the same gate, and formal venue submission, are unaffected and remain open.
2. **Availability row (frontmatter) — applied to the body.** The row's figure (1,986/375/0 at `cd001fa`, dated 2026-08-26) is unchanged as a historical fact but was being read as current. `README.md`'s own ratcheted figure as of 2026-08-30 is 2,664 probes / 470 suites + SDK 36, 0 fail, 7 skipped; a same-day (2026-08-31) unratcheted kind-batch recount reads 2,721 / 476, not yet folded into the ratchet — both are stated with their own dates, per §5.1 rule 1, and neither is asserted as final here. The row is re-anchored as a dated snapshot with a forward pointer, the same discipline §5.2 already uses for the pinned M7 floor table.
3. **CLI verb count (§2.6) — not applied to the body, extending D.3 item 2.** D.3 (2026-08-29) recounted 23 `Command` variants against the 13-row table. Recounted again 2026-08-31 against `crates/gx-cli/src/main.rs`: **25 variants**. Two more than D.3's count: `Tui` (the `gx tui` terminal face, merged 2026-08-30, commit `b37bd1fd`) and `Object`, present at this recount but not named in D.3's list. D.3's own recount is now itself one erratum behind; the table stays unedited, consistent with D.3's choice to leave this row as a standing disclosure rather than a maintained count.
4. **Receipt payload field count (§3.1 / D.3 item 4) — not applied to the body.** D.3 recounted 17 `pub` fields on `ReceiptPayload`. Recounted 2026-08-31 against `crates/gx-witness/src/receipt.rs`: **19 fields**. Two more: `payload_version` (`Option<u32>`) and `engine_version` (`Option<String>`, added 2026-08-30 per the field's own doc comment, `req/910`/`req/38` SS830/`req/919` W8). Table stays unedited, same rationale as D.3 item 4.
5. **Verified unchanged on 2026-08-31, logged for completeness:** the endpoint count (D.3 item 5 — recounted directly against `crates/gx-api/src/lib.rs`: still 23 `.route(` call sites / 22 distinct paths); the 17-crate workspace count and its 13-public/4-private split (recounted against `Cargo.toml`: same four private, `gx-adapter-postgres`, `gx-adapter-mysql`, `gx-mcp-wire`, `gx-confine`); and D.3 item 3's CI-tense disclosure — the public repository still carries no `.github/workflows/` directory, and the GitHub Actions API for `TraceFold/tracefold` lists exactly one run total (a Pages deployment, 2026-08-26), so the CI-billing block D.3 recorded on 2026-08-29 has not been lifted.
...
*End of draft v2.1. Formal publication: as of 2026-08-30 a DOI exists, `10.5281/zenodo.22168558` ("A Mechanical World Model for Agents: Quotient Identity, Three-Valued Gates, and Signed Reversibility"), confirmed 2026-08-31 against the Zenodo record — closing the preprint/DOI leg of this gate (§D.4). Promoting the citations in §6 from [PB] to [PV], and any formal venue submission, remain open and author-gated. (v2.1 note: the trademark/registrar clause that stood here in v2 is restated by §7.7 — the repository and name have been in public use since 2026-08-13; the third-party `.dev` collision remains open and unclaimed either way.)*
