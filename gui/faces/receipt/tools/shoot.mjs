...
// things a photograph is needed to settle. Same discipline faces/ledger's and
// faces/held's own shoot.mjs state: the measurements are not the verdict, the
// picture is -- both are written down, and the numbers exist so a person knows
// where to look, not so nobody has to look.
//
...
// full / tap targets). One addition specific to this face: `sealedMarksDrawn`,
// which must be zero on every capture in this environment -- no verifier is wired
// anywhere in this codebase yet, so parts/src/seal-claim.mjs's claimOf() can never
// return the sealed mark here, and this reading is what would catch it if that
// ever silently stopped being true.
...
const PROBE = `(() => {
  ...
  // receipt's own reading: no verifier is wired anywhere in this codebase, so
  // this face must never draw the sealed mark -- see faces/receipt/README.md.
  const sealedMarksDrawn = [...document.querySelectorAll('svg[data-mark="structure/seal"]')].length;
  return {
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  shoot().then((report) => {
    for (const row of report) {
      process.stdout.write(`${row.fixture} [${row.scheme} ${row.width}px] overlaps=${row.overlapCount} rows=${row.rowsDrawn} repeated=${row.repeatedRows.length} glyphs=${row.glyphs} oversize=${row.oversizeGlyphs.length} filled=${row.filledGlyphs.length} sprites=${row.sprites} overflow=${row.horizontalOverflow} clipped=${row.clippedCells} clippedWithoutFull=${row.clippedWithoutFull.length} sealedMarksDrawn=${row.sealedMarksDrawn} controls=${row.interactiveControls} underTapBudget=${row.underTapBudget.length} textChars=${row.visibleTextChars} bg=${row.background}\n`);
