...
//! no code path in this module that writes another method; `crates/gx-cli/tests/r942_tui.rs`
...
impl Reading {
    ...
    /// the six words exist to prevent, committed by the face rather than by the engine.
    ///
...
pub fn read_route(base_url: &str, path: &str, token: Option<&str>) -> Reading {
    ...
    let read_at = rfc3339(crate::clock::now().0);
    match outcome {
        Ok((status, raw)) => {
...
/// measure it rather than to promise it: `crates/gx-cli/tests/r942_tui.rs` runs this function and
...
/// The coverage grid for the six words, declared rather than inferred.
///
...
pub enum Cell {
    ...
    Value(String),
    /// The wire carried one of the six absences.
    Nothing(Nothing),
}
...
///
/// Built on [`cell`] rather than beside it, so the six words keep their one classifier: the fourth
