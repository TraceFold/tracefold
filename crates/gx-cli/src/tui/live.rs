...
//! The subscription: what the engine pushes, and the four states of being connected to it.
//!
...
//!
//! # The four states, and why `closed` is the one that matters
//!
...
//! | [`Link::Closed`] | it was connected, and what has happened since is not knowable | `?` |
//!
...
//! is happening, which is precisely the collapse (`unknown` into `zero`) that this face's six words
//! exist to refuse. `open` **with** zero events is a different sentence and it is a measurement:
//! this process was listening, and nothing came.
//!
//! 🔴 Three of the four states reuse [`super::wire::Nothing`] rather than spelling marks of their
...
/// like every other mark in this face, and equal to none of the six words for nothing — being
...
pub enum Link {
    ...
    Open,
    /// It was connected, and it is not now.
    Closed,
}

/// Every state a subscription can be in. Gate g19 requires there to be four of them and requires
/// each one to be drawn differently from the other three.
pub const LINKS: [Link; 4] = [Link::Off, Link::Opening, Link::Open, Link::Closed];
...
impl Link {
    ...
    /// nought" would be an answer this process is not entitled to give.
    #[must_use]
    ...
    ///
    /// 🔴 Four sentences and not one with a number in it. A single sentence covering four states is
    /// where the four collapse into two, because the writer of it has to choose which distinction to
    /// spend the words on.
    #[must_use]
...
pub struct LinkReport {
    ...
    /// lie "nothing came".
    pub unreadable: u64,
    /// How many times the connection has been opened again after ending.
    pub reconnects: u64,
}
...
impl Subscription {
    ...
    pub fn start(base_url: &str, token: Option<&str>) -> Self {
        ...
        // 🔴 A thread that could not be started is a subscription that is closed, said out loud. The
        // alternative — leaving the state at `opening` forever — is a screen that waits for
        // something nobody is waiting for.
        if worker.is_none() {
            if let Ok(mut state) = shared.lock() {
                state.link = Link::Closed;
            }
        }
        Self {
            shared,
    ...
    ///
    /// A poisoned lock reports `closed`, which is true in the only sense that matters: nothing will
    /// arrive through it again.
    #[must_use]
...
fn follow(
    ...
    while !stop.load(Ordering::Relaxed) {
        ...
        match wire::open_stream(base_url, STREAM_ROUTE, token, STREAM_TICK) {
            ...
            // An engine that answers and refuses, and an engine that cannot be reached, are the same
            // fact for this line: nothing is arriving. What each of them *means* is already on the
            // screen, in the status codes the four reads carry.
            Ok(_) | Err(_) => {}
        }
        if stop.load(Ordering::Relaxed) {
            return;
