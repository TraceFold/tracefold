...
//!
//! 34's implementation column for both is `M5/M6`, and 51 §15's M6 row is verbatim "`gx escalation (sem: SEM-gx-cli-2259)
//! approve|reject`/`gx cancel` CLI/API wiring re-confirmation" (sem: SEM-gx-cli-2260). M5 measured the engine logic
...
//!
//! Both criteria start at "a Candidate T in `Escalated` state (EscalationTicket already issued)" (sem: SEM-gx-cli-2261), and the engine
...
//! undone: the escrow ceiling (`MAX_INVERSE_PAYLOAD_BYTES`, M4-21 adopted (a); sem: SEM-gx-cli-2262). An inverse carries the whole
...
//! (M6-04 adopted (c); sem: SEM-gx-cli-2263) — because an asymmetry inside one specification is a place two implementations grow
//! apart.
...
/// Get to "a Candidate T in `Escalated` state (EscalationTicket already issued)" (sem: SEM-gx-cli-2264) and hand back the two names.
fn escalated(name: &str) -> (Pipeline, String, String) {
    ...
    assert_eq!(
        verified.code, 4,
        "44 §1.2 `gx verify`: \"4=Escalate\" (sem: SEM-gx-cli-2265). The fs adapter declines an inverse above the escrow \
         ceiling (M4-21 adopted (a)) and E-M3-4 escalates it. stderr: {}",
        verified.stderr
    );
...
fn ac_071_an_approved_escalation_commits() {
    ...
    // submitter)" (sem: SEM-gx-cli-2266), and a suite that ruled with the submitter's own key would be unable to tell a
    ...
    assert_eq!(
        approved.code, 0,
        "44 §1.2 `gx escalation approve`: \"0=success\" (sem: SEM-gx-cli-2267). stderr: {}",
        approved.stderr
    );
    assert_eq!(
        approved.json()["state"],
        "Admitted",
        "34 AC-071: \"T transitions to `Admitted`\" (sem: SEM-gx-cli-2268)"
    );
    // "the issued receipt trail (journal/Receipt metadata) includes `Evidence(HumanDecision)` (decision=
    // Admit, reason, the arbiter's actor)" (sem: SEM-gx-cli-2269). E-M2-3 retired the `Evidence` variant, so the pair is
    // the journal's `HumanDecision` record and the signed `VerdictReceipt` (M5H6-7's reading, which
    // §43 confirmed it; sem: SEM-gx-cli-2270). The receipt count is what the CLI can see; the record is measured by the engine
    ...
    assert_eq!(
        ...
        "🔴 43 T-5 issues a signed receipt and this process holds **one** — not two. T-4c's was \
         ...
         adopted (c) put at \"the moment hand 4's escalation receipt came out\" (sem: SEM-gx-cli-2271). This is that moment, and the number is \
         the material: {}",
        approved.stdout
    );
    assert_eq!(
    ...
    // "from here on, the canonicalize→commit pipeline can continue" (sem: SEM-gx-cli-2272), through the binary.
    ...
    assert_eq!(
        committed.code, 0,
        "34 AC-071: \"from here on, the canonicalize→commit pipeline can continue\" (sem: SEM-gx-cli-2273). stdout: {} stderr: {}",
...
/// 🔴 **AC-072** — reject by **transformation id** (M6-04 adopted (c)'s second spelling; sem: SEM-gx-cli-2274), and it is terminal.
#[test]
fn ac_072_a_rejected_escalation_is_denied_and_terminal() {
    ...
    assert_eq!(
        rejected.code, 0,
        "44 §1.2: \"0=success\" (sem: SEM-gx-cli-2275). stderr: {}",
        rejected.stderr
    );
    assert_eq!(
        rejected.json()["state"],
        "Denied",
        "34 AC-072: \"T transitions to `Denied` and is terminal\" (sem: SEM-gx-cli-2276)"
    );
    ...
    assert_eq!(
        ...
        tid,
        "🔴 M6-04 adopted (c) (sem: SEM-gx-cli-2277): 44 §1.2 writes `<TICKET_ID>` and 44 §2.2 writes `{{id}}`; both name one \
         thing and this surface accepts either"
    );

    // "(outside record-only mode, it cannot proceed further to commit)" (sem: SEM-gx-cli-2278).
    let committed = run(fixture.gx().args(["commit", &tid]));
    println!("AC072_TERMINAL commit_exit={}", committed.code);
    assert_eq!(
        committed.code, 2,
        "44 §1.2 `gx commit`: \"2=Deny, refused because not yet Admitted (non-record-only and Verdict≠Admit)\" (sem: SEM-gx-cli-2279). \
         stdout: {}",
        committed.stdout
    );
    assert_eq!(
...
/// 🔴 A ticket id nothing has raised is 44 §1.2's "6=not-found (unknown ticket)" (sem: SEM-gx-cli-2280).
///
/// The status M6-04 made reachable at all: before `Engine::transformation_of_ticket` a
/// `<TICKET_ID>` resolved to nothing **whatever it named**, so "not-found" and "your ticket is fine
/// and this build cannot find it" (sem: SEM-gx-cli-2281) were the same answer.
#[test]
fn an_unknown_ticket_is_not_found() {
    ...
    assert_eq!(
        ruled.code, 6,
        "44 §1.2: \"6=not-found (unknown ticket)\" (sem: SEM-gx-cli-2282). stderr: {}",
        ruled.stderr
    );
}

/// 🔴 A blank `--reason` is refused before a project is opened (44 §1.2: "ruling reason (required)"; sem: SEM-gx-cli-2283).
#[test]
fn a_ruling_that_says_nothing_is_refused() {
    ...
    assert_eq!(
        refused.code, 1,
        "discipline 52: \"invalid input\" (sem: SEM-gx-cli-2284). AC-071/072 both require the reason to reach the trail, and a refusal \
...
///
/// 42 §3.13's `HumanDecision.actor` is "the ruler, which is **not** `Transformation.actor` (the
/// submitter)" (sem: SEM-gx-cli-2285), and INV-S6 is why an escalation exists at all: it records **who allowed** a change.
