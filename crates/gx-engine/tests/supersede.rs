//! **T-12 / E-M5-9 / 規律48** — the supersede edge from the inside, and the window it leaves open.
//!
...
//! * 🔴 **規律48** — 「終端 record が中間 record の情報を再供給する経路では、中間状態で止まる probe を
//!   必ず 1 本置く」 (§40 M5H3-6). T-12's `Superseded` re-supplies both halves of what a replay knows
...
///
/// **E-M3-4** makes 「`adapter.invert` answered `None`」 the one condition that produces an
/// `Escalate` in v0.1. So a transformation with no constructible inverse stopped at `Escalated`,
/// and hand 4 could write, truthfully, that its `None` arm at T-10b was 「Unreachable in v0.1」.
/// T-5 is what makes it reachable: a person approves the escalation, the transformation walks on to
/// `Committing`, and 43 T-10b's guard 「inverse構成可能（`Some`）」 does **not** open.
///
/// §40 reserved the erratum for exactly this turn:
///
/// > **M5H3-2 採(a)方向・実装窓=手 6**=**E-M5-9(予約)**: `InverseEscrowed.inverse_cid` の `Option`
/// > 化…を、**手 6 の escalation 承認が経路を現実にする同 turn で実装**する
///
...
fn e_m5_9_a_commit_with_no_constructible_inverse_records_the_absence() {
    ...
    assert_eq!(
        ...
        "🔴 E-M5-9: one record, carrying the absence. Hand 4 wrote none at all here, which left \
         「we asked and there is none」 and 「we never asked」 with one face (§32 M4H4-2)"
    );
    assert_eq!(
        engine.inverse_status(&id),
...
fn undone(
    ...
    let (_, t_u) = engine.undo(&t_o, 111, UNDO_AT).expect("the candidate");
    engine
...
///
/// 43 T-12's journal cell is `Superseded{T_o.id, by: T_u.id}` and its side effect is 「`T_o`の
/// メタデータに`superseded_by = T_u.id`を追記」. The record is **about** `T_o` and comes **after**
/// `T_u`'s `Committed`, which is the order 43 T-12's trigger requires (「別Transformation`T_u`が
/// `Committed`へ到達し」) and the reason 規律48's window exists.
#[test]
fn t_12_writes_one_record_about_the_original_after_the_inverse_commits() {
    ...
    assert_eq!(
        after,
        vec!["Superseded"],
        "43 §5-4: 「`T_o`の canonical record・receipt・ledger entryは一切書き換えられない」 -- the \
         one thing appended is the edge"
    );
}

/// 🔴 **規律48**: the state **between** `T_u`'s commit and the supersede edge, measured.
///
/// §40 制定: 「終端 record が中間 record の情報を再供給する経路では、中間状態で止まる probe を必ず
/// 1 本置く」. Here the terminal record is `Superseded`, and it re-supplies two facts a replay would
...
/// and draws no supersede edge, because 43 T-12's guard is 「`T_u.parents`が`T_o.id`を含み」 and the
...
///
/// **E-M5-2** defines AC-039's 「結果状態」 as Σ, and hand 3 measured the agreement over a journal a
...
/// two ever disagreed, 則 1 (「メモリ上の表は cache であって state ではない」) would be false of the
...
fn sigma_agrees_with_its_reconstruction_across_the_supersede_edge() {
    ...
    assert_eq!(
        engine.superseded_by(&t_o),
        Some(t_u),
        "and so does the index M5-09 採(a) names"
    );
    assert_eq!(
        live, replayed,
...
///
/// 「同一`T_u`による重複supersede適用は無視（`superseded_by`が既に設定済みなら再設定しない）」, at the
/// level of the type M5-09 採(a) names. Exercised directly because the engine's own path can only
