...
//! の対に対する決定性+substrate への**書き込み** 0」, adding, in this crate's name: 「読み込みは禁じない
//! ——M7 の git adapter が object store を読む道を閉じない」.
//!
...
//! branch, becomes these bytes」. [`crate::apply`] reads the tip at the moment the engine's CAS has
//! just declared current (41 §5-5b) and builds the commit there.
//!
//! # Why a module rather than a function in `adapter.rs`
//!
//! Because 「calls no repository operation」 is a claim about source, and the cheapest honest way to
...
//! quantifies determinism over 「同一snapshot」; an adapter whose payload is a target state does not
...
///
/// The delta is the one-operation sequence of **M4-13 採(a)**: the entry at the intent's normalised
...
/// content」 — the whole of the interpretation P-6 reserves to an adapter.
///
/// # Errors
...
/// name a repository, a reference and a path inside its tree — 「引数が位置でない」 rather than 「適用に
/// 失敗した」 (**M4H5-5 採(b)**). [`Error::NotDigestible`] when the sequence has no canonical form.
pub fn plan(intent: &Intent, _pre: &ObjectSnapshot) -> Result<PlannedDelta> {
    ...
    // answer different questions — 「that is not a place」 and 「no change reaches that goal from here」.
