...
//! independence and not a difference. The rulings are **M4-25 採(a)+反射=Conflicts** (symmetry, and
//! `commutation(a,a)` is `Conflicts`), **M4-14** (the residual's referent), **E-M4-8** (a payload is
//! kept, which is what gives the referent somewhere to be) and **M4-13 採(a)** (the grammar that makes
//! the question decidable at all).
//!
//! # The whole question, in one line of this grammar
//!
//! Under **M4-13 採(a)** every operation is a whole-file replacement or a whole-file removal, so an
...
//! one path are one file (**E-M4-12**; M3-10 fixed a policy pack's effective range at 「locator 級」, so
...
//! and it is what makes AC-053's 「engineパイプライン外」 easy: there is nothing a pipeline could supply.
//!
...
//! 「held back」 is a fact about an order: 43 §8 has the engine keep `T2` in a waiting queue with
...
//!   that 「M4-25 says nothing about the two directions naming the same obstruction」 -- which this
...
//!
//! 42 §3.6 calls it 「独立でない部分」. For whole-file replacement there is no proper part: if two
...
//!
//! **M4-14** asked for 「residual CID が test harness 内で解決可能」 and this is what answers it. There
//! is no delta store in M4 (that is the engine's, in M5), so 「解決可能」 cannot mean a lookup; it means
