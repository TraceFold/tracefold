...
//!
//! req/60 §5.2, 手 2 の DoD: 「**負例**: 同一 policy+同一 request で決定が揺れない事(決定性)を
//! property で」. What that property is worth depends on what could make it fail, and the answer is
...
///
/// `include_str!` would be a second road into the file, and FR-028's 「pack の file が build に
/// 埋め込まれる経路が 1 本」 is hand 5's check to make. Today the count of embedded paths is zero,
...
proptest! {
    ...
    ///
    /// # Rewritten by hand 4, and why that is not 「テストを通すためにテストを変えた」
    ///
    /// Hand 2 wrote this with two arms and a sentence saying the third was unreachable: 「**E-M3-4**
    ...
    /// supposed to fall**, which is the shape req/38 §21's C-9 and §22's D-11 already追認'd for
...
/// decided」 -- so that a pack could not impersonate the absence of a decision. C-3 replaced the
...
/// policy that grants it」 (docs.cedarpolicy.com/auth/authorization.html).
#[test]
...
/// wired a set up」 are different facts, and only the first one is a verdict. **E-M3-3**.
#[test]
