...
pub enum Error {
    ...
    /// values below it, so 「negative」 is representable and means a moment that is not one of the
    ...
    CreatedAtNegative { got: i64 },

    /// **E-M3-13** ②: `intent_id` is the all-zero placeholder.
    ///
    /// The all-zero id is this crate's own spelling for 「no value yet」 -- `PROVISIONAL_ID` in
    /// `transformation.rs` is those thirty-two bytes -- and 42 §1.3 puts `intent_id` **inside** the
    ...
    CreatedAtBeforeParts { got: i64, at_least: i64 },

    /// **E-M4-15**: two fingerprints whose `scope` differs were compared.
    ///
    /// 42 §3.5 逐語: 「`Fingerprint`同士の等価は `substrate` と `scope` が一致し、かつ `digest` の
    /// バイト列が一致することと定義する(`scope`不一致の比較は意味を持たないため**adapter実装エラーと
    /// して扱う**)」. A `bool` cannot carry that sentence -- `false` would read as 「the state moved」
    /// and the CAS check of CON-2 would abort a commit for a reason that never happened -- so
    /// [`crate::Fingerprint::cas_eq`] returns this instead, which is E-M4-15's 「型形」.
    ///
    ...
    FingerprintScopeMismatch { left: String, right: String },
    ...
    /// 「adapter実装エラー」 -- and raised the asymmetry (req/70 §2 M4H1-1). req/38 §29 ruled case (b):
    ///
    /// 「`cas_eq` は **substrate 不一致も `Err`**(scope 不一致と同じ「実装エラー」級——別 adapter の
    /// 指紋比較は engine 配線 bug であり、`Ok(false)`→「状態が動いた」Abort に化けると bug が隠れる
    /// =E-M4-15 が PartialEq を拒んだのと同じ論法)。42 §3.5 の等価性は「**同一 adapter の産物間でのみ
    /// 定義**」と読む」
    ///
    ...
    /// **M4H1-2 採(a)**: a `scope` longer than [`crate::MAX_SCOPE_BYTES`] reached
    /// [`crate::Fingerprint::new`].
    ///
    /// req/38 §29 逐語: 「gx-core にも scope 文字列の上限+超過時 digest 化(gx-gate M3-21/ASM-60-4 と
    /// 同形・1024 byte)。**scope の生成規則(ASM-69-1)が実体化する手4 と同窓**で入れる(生成と上限を別の
    /// 手で決めない)」.
    ///
    ...
    /// unbounded string in a signed record is 42 §3.8's 「長文は digest 化」 problem one type over.
    /// What the ruling calls 「超過時 digest 化」 cannot happen *here* -- A-1 keeps every digest in
    /// gx-canon and this crate depends on no encoder -- so the two halves are split the way E-M2-1
    /// splits everything else: 型と上限は下層(this refusal), digest 化は上層
    ...
    #[error(
        "a fingerprint scope of {bytes} bytes exceeds the {max}-byte bound (M4H1-2); an adapter \
         elides it to a digest before constructing (E-M4-1: 計算は上層)"
    )]
    ScopeTooLong { bytes: usize, max: usize },
}

/// The crate's error vocabulary, declared once (**E-M2-23**, **H-3**).
///
/// req/38 §25 逐語: 「**H-3(M4 窓)**: gx-core の Error 語彙表は次に gx-core を触る手(M4 冒頭=H-1
/// 実装と同窓)で E-M2-23 形に」. `gx-gate`'s `ERROR_KINDS` is the先例 (req/64 §2), and E-3 (req/38
/// §23) is the ruling that the table lives beside the enum rather than in 41 §6: 「E-M2-23 の「41 §6
/// に語彙表」は spec 凍結(52)の下では**書けない**——erratum の正本は req/38 台帳であり、code 側
/// `ERROR_KINDS`+突合 test が実効体」.
///
/// # What being outside this table would mean, and why it cannot happen
///
/// gx-gate's vocabulary is a set of strings a caller supplies, so 「宣言外の code は構成時拒否」 is a
