...
//!
//! `req/38_ERRATA_2026-08-07.md` §9 left one question to hand 5: "the JSON spelling of raw bytes
//! (sig / FingerprintBytes) is **decided by hand 5, the receipt's JSON face**; the default
//! recommendation is base64, the same family as 42 §1.2's Cid and 44 §2.2's payload" (quoted in
//! SEM-gx-core-003). Hand 5 takes the recommendation. 44 §2.2 already spells one raw byte string
//! in base64 -- "`Receipt` (the JSON representation of 42 §3.10; `payload` is base64)" -- and a
//! workspace where the payload is base64 while the signature over it is a list of sixty-four
//! integers would have two answers to one question. The DAG-CBOR face is untouched and stays a byte
//! string (major type 2), which `req/38_ERRATA_2026-08-07.md` §9 fixes as "already decided; do not
//! move it".
//!
...
//! and 41 §2 allows this crate "serde, thiserror and not much more" (sem: SEM-gx-core-004), which a
//! codec dependency is not. The
...
//! §9) already ruled on exactly this choice when hand 1 added four: "the proposal to keep piling
//! into lib.rs is rejected on readability-first (discipline 3) -- the lane's four added modules are
//! confirmed" (quoted in SEM-gx-core-006). 41 §2's module list for gx-core, extended from
...
pub fn decode(text: &str) -> core::result::Result<Vec<u8>, &'static str> {
    ...
    // `is_multiple_of` rather than `% 4 != 0`: blocker item 1 (req/38 §47; sem: SEM-gx-core-007)
    // raised the declared MSRV from 1.85
