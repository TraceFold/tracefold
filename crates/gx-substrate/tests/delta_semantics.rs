//! **M4-07 採(c)** — the composite-delta section of the crate documentation, measured clause by
//! clause.
//!
//! `req/38_ERRATA_2026-08-07.md` §28 逐語: 「**M4-07 採(c)**(数学・D-8 の穴の決着): 合成 delta は
//! **free monoid**——fs payload を「単一 file 操作の列」とし、列の連結が合成の witness。trait は 1 行も
//! 変えず(P-6 無傷・F1 不発)。**一般法則(合成 arrow の delta=部分の合成)は主張しない**事を doc に明記」.
//!
...
/// below survived mutation (a) of `tools/verify_m4h3.sh`: gx's own sentence 「一般法則(合成 arrow の
/// delta=部分の合成)は主張しない」 was rewritten into a description, and every clause was still found --
/// in the block quote of **M4-07 採(c)** three paragraphs above, where the same words appear because
/// the section quotes the ruling verbatim.
///
/// That is req/71 §2 M4H2-6's lesson arriving a second time: 「**「どこかに書いてある」は「その場所に
/// 書いてある」ではない**」. A quoted ruling is evidence of what was decided; what binds an implementor
...
/// What **M4-07 採(c)** decided, each clause word for word.
///
...
///
/// 🔴 The seventh token is **K-2 採(a) の最小形** (`req/38` §35), and it was earned by a mutation the
/// audit hand fired at this section. req/76 §2.1 (B-2) rewrote numbered clause 4 into 「T2 は本
/// milestone で確立した」 and **every probe in this file stayed green**: the six tokens above are all
/// about clauses 1-3, and the one probe that mentions T2 asks only that the string `T2` appear --
/// which an overclaim keeps. So 45 §4's 「言ってはならない文」 was, in this section, unguarded in the
...
const RULING_CLAUSES: [&str; 7] = [
    ...
    "単一 file 操作の列",
    "列の連結が合成の witness",
    "一般法則(合成 arrow の delta=部分の合成)は主張しない",
    "trait は 1 行も変えず",
    "M4-07",
...
const CONTRAST_CLAUSES: [(&str, &str); 3] =
    [("darcs", "全域"), ("pijul", "pushout"), ("git", "snapshot")];
...
/// literature review.
const GX_POSITION: [&str; 2] = ["gx の逆=部分", "合流を解かず Conflicts で止める"];
...
fn the_composite_delta_ruling_has_a_section_of_its_own() {
    assert!(
        ...
        "the crate documentation has no `{SECTION_HEADING}` section; M4-07 (c) asks for the \
         decision to be written 「doc に明記」 and a decision without a place is a decision without \
         a reader"
    );
}
...
fn the_section_states_the_free_monoid_ruling() {
    ...
    assert!(
        missing.is_empty(),
        "the composite-delta section does not state {missing:?} -- M4-07 採(c) is the decision that \
...
///
/// Separated from the loop above because it is the load-bearing half of the ruling. 「合成 arrow の
/// delta と部分の delta の関係」 is exactly what req/69 §4 M4-07 measured as undefined: the trait has
...
fn the_section_refuses_the_general_law_rather_than_omitting_it() {
    let doc = ruling_section(&lib_rs());
    assert!(
        doc.contains("一般法則(合成 arrow の delta=部分の合成)は主張しない"),
...
fn the_section_places_gx_against_the_three_patch_theories() {
    ...
    assert!(
        missing.is_empty(),
        "the comparison is missing {missing:?} -- req/69 §9 items 1-3 exist so that 「採らない理由」 \
...
fn the_boundary_crate_still_reads_no_payload() {
    ...
    assert!(
        ...
        "the boundary crate decodes something: {offenders:?}. M4-07 (c) puts the composite in the \
         payload precisely because 「trait は 1 行も変えず(P-6 無傷)」, and a decoder here would be \
