//! The seven contracts of `SubstrateAdapter`, measured as written obligations.
//!
//! req/69 §6.2 hand 2 逐語: 「**7 method それぞれの契約文を 1 表で書く**——特に「冪等はどの量化か」
//! 「plan は何に対して決定的か」を逐語で」. This suite is what keeps that table a requirement rather
...
//!
//! req/69 §3.2 shows in three lines that reading 41 §4's 「適用は冪等」 and AC-049's round trip as
//! laws about a state map at once collapses every delta to the identity:
//!
//! ```text
//! ⟦δ⁻¹⟧∘⟦δ⟧ = id (全域) かつ ⟦δ⟧∘⟦δ⟧ = ⟦δ⟧ (全域) ならば ⟦δ⟧ = id
//! ```
//!
//! **E-M4-3** closed that by quantifying each one narrowly -- idempotence over 「同一 delta の再入
//! (retry)」 and the round trip over 「`invert` に渡した `pre` の 1 点」 -- and **E-M4-4** did the same
//! for `plan`, whose determinism is 「(intent, snapshot) の対に対して」 rather than over an intent
...
/// quantifier were written under the wrong method, and a table that says 「同一 delta の再入」 about
/// `plan` is worse than one that says nothing, because it reads as a decision.
const REQUIRED_CLAUSES: [(&str, &[&str]); 7] = [
    ("kind", &["産物の `substrate` はすべてこの値"]),
    ("snapshot", &["locator は正規化済みで受ける"]),
    ("plan", &["(intent, snapshot) の対に対して", "副作用なし"]),
    (
        "precondition",
        &["同一 adapter の産物間でのみ定義", "変更前後で異なる値"],
    ),
    ("apply", &["同一 delta の再入"]),
    (
        "invert",
        &["`invert` に渡した `pre` の 1 点", "上限超過は `Ok(None)`"],
    ),
    (
        "commutation",
        &[
            "commutation(a,b) == commutation(b,a)",
            "commutation(a,a) は `Conflicts`",
        ],
    ),
];
...
/// quantifier, and the words 「同一 delta の再入」 were still found -- in the third column, where the
