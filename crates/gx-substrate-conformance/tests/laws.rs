...
//! in one report. (sem: SEM-gx-substrate-conformance-069, SEM-gx-substrate-conformance-070,
//! SEM-gx-substrate-conformance-071, SEM-gx-substrate-conformance-072, SEM-gx-substrate-conformance-073,
//! SEM-gx-substrate-conformance-074, SEM-gx-substrate-conformance-075, SEM-gx-substrate-conformance-076)
//!
//! req/38 §28, verbatim: "**the survival ruling for L1-L8**: all eight adopted", and §30 M4H2-4 added
//! K1. L8 is not here --
...
fn the_mock_adapter_obeys_every_law() {
    ...
    assert_eq!(Origin::Law.label(), "law");
}
...
/// is quantified at "the one pre-state point handed to `invert`" and §28 made writing that state
/// into the Given a
...
/// A bare fixture leaves L5, L6 and L7 "NOT_SUPPLIED", and the run is not conformant.
#[test]
fn the_laws_that_need_a_subject_say_so_when_there_is_none() {
    ...
    // §31 M4H3-4 (b), implemented in M4 hand 4: "zero failures" and "zero unmeasured" are two
    // questions.
...
fn run_all_keeps_the_two_sections_distinguishable() {
    ...
    assert!(
        report.is_complete(),
        "the full fixture supplies every subject, so \"zero unmeasured\" holds too (§31 M4H3-4 (b))"
    );
