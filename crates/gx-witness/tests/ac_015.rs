//! AC-015 (FR-015) — `Provenance::derive_from` is a deterministic derivation.
//!
//! AC-015 逐語: 「Given: 既存Committed Transformation T。When: `Provenance::derive_from(&T)`を2回
//! 呼ぶ。Then: 2回とも同一のProvenance値（決定的導出）。」判定方法 `unit + property`, M2.
//!
...
//! `input_objects`（「plan/verify中にadapterが読み取った入力スナップショット群」）and an
...
//! way E-A7-2 ruled composition: 「導出不能分は呼び出し側引数(`ProvenanceInputs`)」. So the call is
...
//! req/49 §3 M2-19 raised (「`Vec` の順序が CID に効くのに正準規則が無い」) and what req/26 §2's
...
proptest! {
    /// The property half of 34's 判定方法 column. Many shapes of arrow and input set, each derived
...
/// and — once anything hashes one — two identities. req/49 §3 M2-19's 既定案 is 「収集順でなく id
/// 昇順に正準化する」, which is req/26 §2's id-sort-for-determinism, and this is the assertion of it.
#[test]
...
/// 42 §3.9 is a sequence, and 「adapterが読み取った入力スナップショット群」 does not say whether
...
/// 42 §3.9 逐語: `transformation: TransformationId`, `intent_digest: Option<Cid>`,
...
/// 42 §3.9's `Environment` 逐語: `engine_version: String`, `adapter_kind: SubstrateKind`,
...
/// 42 §1.3-3 逐語: 「`IntentId`は`Intent`（§3.3）のCIDであり`submit`時点（43 T-1、Draft遷移）で
/// 確定」. So 42 §3.9's 「`submit`時のIntentのcanonical digest」 is exactly `t.intent_id`'s digest —
...
/// 42 §3.9 逐語: 「Draft/Candidate生成時のみ設定（order≥1の合成変換ではNone）」. The predicate this
...
/// arrows therefore yields an order-0 arrow, so 42 §3.9's parenthetical 「order≥1の合成変換」
