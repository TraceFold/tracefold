...
fn dr_46_28_the_payload_declares_a_boundary_field_that_is_not_optional() {
    ...
    assert_eq!(
        fields, 17,
        "DR-46-26 took the count to fourteen, DR-46-28 added the fifteenth, S③ (`req/493`          §1 AC-6) the sixteenth, and DR-46-39 (`req/777` catalogue_hash) the seventeenth"
    );
}
...
fn the_wire_moved_by_exactly_the_one_key_dr_46_28_added() {
    ...
    assert!(
        now.starts_with("b1"),
        "a canonical map of seventeen opens `b1`; today's opens {}",
        &now[..2]
    );
