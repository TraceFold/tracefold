...
| **What it refuses to do** | **Receipt soundness and witness composition are not proved.** `Receipt.lean` does not exist — you can check that yourself in [`lean/`](../../lean) — and a receipt this crate issues is a signed record of what the engine decided, nothing stronger. Saying otherwise is exactly the overclaim this project forbids itself, so it is written here rather than left for a reader to discover. |
