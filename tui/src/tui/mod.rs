...
// 🔴 An [`Answer`](crate::Answer) and not an `Outcome`. Until #188/#189 this module returned
// `gx_cli::exit::Outcome`, which carries an exit code — a word in the CLI's vocabulary that a face
// has no business minting. Both roads below only ever produced `Outcome::ok`, so the consumer's
// mapping is `Outcome::ok(answer.json)` and the process's status is unchanged; what moved is who
// gets to say it.
use crate::Answer;
use renderer::Tier;
use wire::Screen;
...
pub fn run(options: &Options) -> crate::Result<Answer> {
...
/// One frame, off-screen, composed.
///
/// 🔴 **Composed and not printed**, which is the one behavioural line the extraction moved. This
/// used to call `crate::say!` per row; a package that is not the CLI must not hold a standard
/// output stream, because `probes/doubt/tests/declaration_writer_doubt.rs` d6 counts those sites
/// inside `crates/gx-cli/src` and a writer over here would be invisible to it rather than merely
/// red. The rows come back in [`Answer::lines`] and `gx-cli`'s existing `say!` site puts them out.
fn dump(options: &Options, tier: Tier) -> crate::Result<Answer> {
    ...
    let plan = layout::resolve(
        ...
        &measured,
        options.wide,
        // `--dump` draws one frame from a view nobody has moved, so the subject is the grid. It is
    ...
    let lines: Vec<String> = renderer::buffer_text(&buffer)
        .lines()
        .map(str::to_string)
        .collect();
    Ok(Answer {
...
fn interactive(options: &Options, tier: Tier) -> crate::Result<Answer> {
    let (frames, reads) = renderer::interactive(options, tier)?;
    // No lines: the terminal already showed the frames, and re-printing them after giving the
    // screen back would be a second, worse copy of what the reader just watched.
    Ok(Answer {
        lines: Vec::new(),
        json: json!({
            "gx": "tui",
